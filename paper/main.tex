...
\begin{abstract}
Vector quantization (VQ) of the key-value (KV) cache, exemplified by TurboQuant, promises near-lossless compression at low bit-widths through random rotation and Lloyd-Max coding. We present the first comprehensive cross-architecture evaluation of TurboQuant across five modern LLMs spanning three grouped query attention (GQA) ratios, and surface a critical failure mode: \emph{GQA amplification}, where compression error at a shared KV head propagates to all query heads in the group. On Gemma-3-27B (GQA 2:1), TurboQuant is near-lossless at 4 bits with only a $+0.7\%$ perplexity increase---$30\times$ closer to FP16 than KIVI's $+22.4\%$. On Mistral-7B and Llama-3.1-8B (GQA 4:1), TurboQuant retains a $8$--$9\times$ advantage over KIVI. On Qwen2.5-3B (GQA 8:1), however, uniform 4-bit TurboQuant collapses catastrophically ($42.3$ vs.\ $7.6$ for FP16). We propose two complementary contributions to rescue VQ in the high-GQA regime: \emph{(i) sensitivity-adaptive per-layer bit allocation} via a dynamic programming formulation that minimizes total distortion under an average-bit budget, informed by offline calibration; and \emph{(ii) outlier-aware channel grouping}, which stores a small set of high-magnitude channels in FP16 and quantizes the remainder. On Qwen2.5-3B, outlier-aware compression achieves $7.74$ PPL at an effective 4.3-bit budget---a $5.5\times$ improvement over uniform K4V4, and within $1.5\%$ of FP16. Adaptive allocation further extends usable compression down to $2.5$ bits on average. Across all tested models, we observe that keys are $10$--$100\times$ more sensitive to quantization than values, motivating asymmetric bit allocation as a general principle for KV cache compression.
\end{abstract}
...
We find, through systematic replication across five modern architectures, that this rosy picture does not generalize. On high-GQA models---specifically, Qwen2.5-3B and Llama-3.1-70B, which share $2$ or $8$ KV heads among $16$ or $64$ query heads---uniform 4-bit TurboQuant fails catastrophically, inflating perplexity by $5$--$10\times$. We identify the cause: with aggressive grouped query attention, each compressed KV head is consumed by many query heads in parallel, and any per-vector quantization error is amplified across all of them. We term this effect \emph{GQA amplification}.

This paper makes the following contributions:
\begin{itemize}
    \item \textbf{A cross-architecture evaluation of TurboQuant} on five LLMs (Gemma-3-27B, Mistral-7B, Llama-3.1-8B, Qwen2.5-3B, Llama-3.1-70B) covering three GQA ratios. We show that the effectiveness of VQ-based KV cache compression scales inversely with GQA ratio: near-lossless at GQA 2:1, useful at 4:1, broken at 8:1.
    \item \textbf{Sensitivity-adaptive per-layer bit allocation}, which we formulate as a dynamic program that minimizes total quantization distortion subject to an average-bit budget, informed by a short offline calibration. The allocator is aware of asymmetry between keys and values, which our measurements reveal to be $10$--$100\times$ different in sensitivity.
    \item \textbf{Outlier-aware channel grouping}, which detects high-magnitude KV channels at calibration time, stores them in FP16, and applies TurboQuant to the remaining channels. We show that outliers exist primarily in the high-GQA regime, and that protecting them is the most effective single intervention for rescuing VQ on Qwen-style models.
    \item \textbf{Empirical demonstration of rescue.} On Qwen2.5-3B, our methods reduce 4-bit perplexity from $42.3$ (uniform TurboQuant) to $7.7$ (outlier-aware), within $1.5\%$ of the FP16 baseline of $7.6$. Adaptive allocation extends usable compression below 3 bits.
    \item \textbf{Needle-in-haystack validation.} On Mistral-7B, TurboQuant at $4$ key bits / $4$ value bits matches FP16 exactly on needle retrieval across contexts up to $16$K tokens.
\end{itemize}

Our code, calibration profiles, and all experimental configurations are released at \url{https://github.com/azamkhan555622848/Sensitivity-Adaptive-KV-Cache-Compression-TurboQuant}.
...
\paragraph{Models.} We evaluate on five recent open-weight LLMs spanning three GQA ratios: Gemma-3-27B-it (GQA $2{:}1$), Mistral-7B-Instruct-v0.3 and Llama-3.1-8B-Instruct (GQA $4{:}1$), Qwen2.5-3B-Instruct and Llama-3.1-70B-Instruct (GQA $8{:}1$). All models are loaded in BF16 on $3\times$ NVIDIA RTX A6000 ($48$ GB each), giving $144$ GB of aggregate VRAM. For Llama-3.1-70B, the model weights alone consume nearly all available memory; we are only able to run the FP16 baseline and report compressed perplexity for the other four models.
...
\emph{First,} on the two GQA-$4{:}1$ models, TurboQuant-V3 at $4$ key / $4$ value bits is near-lossless ($+0.4\%$ on Mistral-7B, $+1.6\%$ on Llama-3.1-8B) and decisively beats KIVI-4 ($+3.9\%$ and $+12.4\%$, respectively). The gap widens further at $2$ bits, where KIVI-2 collapses entirely but TurboQuant-V3 K2V2 remains usable.
...
\emph{Third,} on Qwen2.5-3B (GQA $8{:}1$), uniform TurboQuant-V3 K4V4 collapses entirely, with perplexity inflating by $4.5\times$. KIVI-4 survives, with only a $+3.4\%$ increase. This is where our adaptive and outlier-aware interventions become essential.
...
We have presented the first cross-architecture evaluation of TurboQuant, a vector-quantization-based KV cache compressor, across five modern LLMs. Our central finding is that VQ's effectiveness scales inversely with the GQA ratio: near-lossless on Gemma-3-27B (GQA $2{:}1$), strongly competitive on Mistral-7B and Llama-3.1-8B (GQA $4{:}1$), and catastrophic on Qwen2.5-3B and Llama-3.1-70B (GQA $8{:}1$). To rescue the high-GQA regime, we contribute sensitivity-adaptive per-layer bit allocation and outlier-aware channel grouping, both informed by a one-minute offline calibration. Together these techniques reduce Qwen2.5-3B $4$-bit perplexity from $42.3$ to $7.7$---within $1.5\%$ of FP16. Our sensitivity analysis also surfaces a general principle: across all tested models, keys are $10$--$100\times$ more sensitive to quantization than values, and layer $0$ keys are consistently the single most sensitive tensor in the cache. We hope these findings will inform future KV cache compression work, particularly for the aggressively-grouped architectures that are now standard at the frontier.
